...
function toCard(job, applicants) {
  return {
    ...
    /*
     * الحقول الاختيارية (الراتب/الموقع/ساعات العمل/نوع الدوام) بتترّجع زي ما
     * هي — من غير أي نص بديل زي "يُحدد لاحقاً" أو "-". العميل طلب إن الحقل
     * الفاضي يختفي خالص من الكارت، فمنسلم Cart القيمة فاضية وهي اللي تخفيها.
     */
    salary: formatSalary(job.salary_from, job.salary_to),
    location: cleanOptional(job.location),
    schedule: cleanOptional(job.schedule),
    type: cleanOptional(job.employment_type),
    required: Number(job.required_count) || 0,
    applicants: Number(applicants) || 0,
    description: job.description || "",
    experience: cleanOptional(job.experience),
    qualification: cleanOptional(job.qualification),
  };
}

/** بترجّع نص نضيف، أو null لو القيمة فاضية/مسافات/شرطة فاضية */
