...
/**
 * بناء رابط واتساب من أي رقم تليفون (بيستخدمه الموقع والأدمن).
 *
 * 🐛 السبب: رقم التليفون بقى بيتقرأ من site_settings (CMS) — فلازم رابط
 * الواتساب يتبني من **نفس الرقم** مش من الرقم الثابت في الكود، وإلا العميل
 * يغيّر الرقم من اللوحة والرابط يفضل رايح على الرقم القديم.
 *
 * بنشيل أي مسافات/شرطات/أقواس، وبنحوّل الرقم لمصري دولي (20 + الرقم من
 * غير صفر البداية). لو الرقم فاضي أو مش صالح بنرجع الرابط الافتراضي.
 */
export function buildWhatsAppLink(rawNumber) {
  const digits = String(rawNumber || "").replace(/[^\d]/g, "");
  const international = digits.startsWith("20")
    ? digits
    : digits.startsWith("0")
      ? `20${digits.slice(1)}`
  ...
  const target = international || `20${CONTACT.whatsappNumber.substring(1)}`;
  return `https://wa.me/${target}?text=${encodeURIComponent(WHATSAPP_TEXT)}`;
}

// رقم دولي بصيغة الواتساب (مصر 20 + الرقم بدون صفر البداية)
export const WHATSAPP_LINK = buildWhatsAppLink(CONTACT.whatsappNumber);
