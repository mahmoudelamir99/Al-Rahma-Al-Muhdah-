...
function Stepper({ current }) { return <div className="mt-5 grid grid-cols-4 gap-1">{stepLabels.map((label, index) => <div key={label} className="min-w-0 text-center"><div className={`mx-auto flex h-7 w-7 items-center justify-center rounded-full text-xs font-extrabold ring-2 ${index < current ? "bg-brand-500 text-white ring-brand-200" : index === current ? "bg-brand-700 text-white ring-brand-200" : "bg-white text-slate-500 ring-slate-200"}`}>{index < current ? "✓" : index + 1}</div><p className={`mt-1 text-[10px] font-bold leading-5 sm:text-[11px] ${index === current ? "text-brand-700" : "text-slate-500"}`}>{label}</p></div>)}</div>; }
...
export default function ApplyModal({ job, requestMode = false, onClose }) {
  ...
  return <ModalPortal><div className="fixed inset-0 z-[9999] flex items-end justify-center bg-black/60 backdrop-blur-sm sm:items-center" onClick={onClose} role="dialog" aria-modal="true" aria-label="نموذج التقديم"><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .22 }} onClick={(event) => event.stopPropagation()} className="fixed bottom-0 left-0 right-0 z-[9999] w-full max-h-[90vh] overflow-y-auto overscroll-contain rounded-t-3xl bg-white p-6 pb-[max(1.5rem,env(safe-area-inset-bottom))] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] sm:relative sm:bottom-auto sm:left-auto sm:right-auto sm:max-w-2xl sm:rounded-2xl sm:p-8 sm:shadow-2xl">
    ...
    {status === "success" ? <div className="py-10 text-center"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-whatsapp text-3xl font-bold text-white">✓</div><h4 className="mt-5 text-xl font-extrabold text-slate-900">تم إرسال بياناتك بنجاح</h4><p className="mt-2 text-sm font-semibold text-slate-600">{isCustomRequest ? "سنتواصل معك عند توافر وظيفة مناسبة." : "سنتواصل معك قريباً."}</p></div> : <><Stepper current={step} /><div className="mt-5 sm:mt-6 sm:min-h-[245px]">
      ...
      {step === 3 && <div className="space-y-4"><div className="rounded-2xl border border-brand-100 bg-brand-50 p-4"><p className="text-sm font-extrabold text-brand-800">الوظيفة المطلوبة</p>{isCustomRequest ? (<><SelectInput name="selectedJob" label="اختر الوظيفة" values={jobOptions} form={form} errors={errors} setField={setField} disabled={isSending} />{form.selectedJob === CUSTOM_JOB_OPTION && <TextInput name="customJob" label="اكتب الوظيفة التي تريدها" form={form} errors={errors} setField={setField} placeholder={CUSTOM_JOB_PLACEHOLDER} disabled={isSending} />}</>) : (<p className="mt-1 text-lg font-extrabold text-brand-950">{job.title}</p>)}</div><p className="text-sm font-semibold leading-7 text-slate-600">راجع بياناتك قبل الإرسال. بياناتك محفوظة بسرية تامة.</p></div>}
    </div><AnimatePresence>{status === "error" && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} role="alert" className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs font-bold text-red-700">{serverError}</motion.p>}</AnimatePresence><div className="mt-5 flex items-center justify-between gap-3 border-t border-slate-200 pt-4"><button type="button" onClick={step === 0 ? onClose : () => setStep((current) => current - 1)} disabled={isSending} className="w-full max-w-sm rounded-xl bg-slate-100 px-6 py-3 text-xs font-bold text-slate-700 disabled:opacity-60">{step === 0 ? "إلغاء" : "السابق"}</button>{step < 3 ? <button type="button" onClick={next} disabled={isSending} className="w-full max-w-sm rounded-xl bg-brand-600 px-6 py-3 text-xs font-bold text-white disabled:opacity-60">التالي</button> : <button type="button" onClick={submit} disabled={isSending} className="w-full max-w-sm rounded-xl bg-whatsapp px-6 py-3 text-xs font-bold text-white disabled:opacity-60">{isSending ? "جارٍ الإرسال..." : "تأكيد وإرسال"}</button>}</div></>}
