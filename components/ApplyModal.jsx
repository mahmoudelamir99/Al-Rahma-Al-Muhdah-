...
function TextInput({ name, label, form, errors, setField, type = "text", placeholder, disabled, inputMode, maxLength, minLength, pattern, dir }) { return <Field label={label}><input name={name} type={type} value={form[name]} onChange={(e) => setField(name, e.target.value)} placeholder={placeholder} disabled={disabled} inputMode={inputMode} maxLength={maxLength} minLength={minLength} pattern={pattern} dir={dir} className={fieldClass(errors[name], disabled)} /></Field>; }
...
export default function ApplyModal({ job, requestMode = false, onClose }) {
  ...
  /*
   * تحقق خطوة واحدة — بيفحص الحقول الإلزامية في الخطوة دي،
   * + قاعدة الرقم القومي (14 رقم بالظبط ومستحيل يتبعت فاضي).
   */
  const validateStep = (stepIndex, currentForm) => {
    const next = {};
    requiredFields[stepIndex].forEach((field) => {
      if (!String(currentForm[field] || "").trim()) next[field] = true;
    });
    // الرقم القومي: إجباري + 14 رقم بالظبط (أرقام إنجليزية فقط)
    if (stepIndex === 0) {
      const nationalId = String(currentForm.nationalId || "").trim();
      if (!nationalId || !/^\d{14}$/.test(nationalId)) next.nationalId = true;
    }
    return next;
  };
  const validate = () => { const next = validateStep(step, form); setErrors(next); return Object.keys(next).length === 0; };
  /*
   * تأمين قبل الإرسال: بنعيد فحص **كل** الخطوات مرة واحدة (مش الخطوة الحالية بس).
   * ده اللي بيمنع إن طلب يتبعت والرقم القومي ناقص أو مش 14 رقم — حتى لو
   * المستخدم وصل للخطوة الأخيرة بأي طريقة.
   */
  const validateAll = () => {
    const next = {};
    requiredFields.forEach((_, index) => Object.assign(next, validateStep(index, form)));
    return next;
  };
  ...
  const submit = () => { if (isSending) return; const allErrors = validateAll(); if (Object.keys(allErrors).length > 0) { setErrors(allErrors); // نرجّع العامل لأول خطوة فيها خطأ (الرقم القومي مثلاً)
      const firstBadStep = requiredFields.findIndex((fields) => fields.some((f) => allErrors[f]));
      if (firstBadStep >= 0 && firstBadStep !== step) setStep(firstBadStep);
      setStatus("error");
      setServerError("في بيانات ناقصة — تأكد من الرقم القومي (14 رقم) وكل الحقول الإلزامية.");
      return;
    } setStatus("sending"); setServerError(""); startTransition(async () => { const result = await submitApplication({ full_name: form.name, phone_number: form.phone, national_id: form.nationalId, age: form.age, gender: form.gender, marital_status: form.maritalStatus, governorate: form.governorate, city: form.city, education_level: form.education, specialization: form.specialization, military_status: form.militaryStatus, experience_years: form.experienceYears, previous_companies: form.previousCompanies, expected_salary: form.expectedSalary, selected_job: resolvedJob }); if (!result?.ok) { setStatus("error"); setServerError(result?.error || "تعذّر إرسال الطلب، جرّب تاني."); return; } setStatus("success"); closeTimer.current = window.setTimeout(onClose, 3000); }); };
